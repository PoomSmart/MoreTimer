...
NSInteger numberOfMenuItems(){
    if (!IS_IPAD && effectiveTimerCount == 3)
        return 5;
...
static void _indicateCaptureTimerProgressUsingTorch(CAMViewfinderViewController *self){
    if (!shouldUseTorch)
        return;
    if (self._currentDevice != 0 || ![(CAMCaptureCapabilities *)[%c(CAMCaptureCapabilities) capabilities] isTorchPatternSupportedForDevice:self._currentDevice])
        return;
    CAMTorchPatternController *torch = MSHookIvar<CAMTorchPatternController *>(self, "__torchPatternController");
    NSInteger totalDuration = isiOS10Up ? [self timerDuration] : [self _timerDuration];
